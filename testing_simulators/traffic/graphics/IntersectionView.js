...
export class IntersectionView extends Intersection 
{
    draw(ctx) 
    {
        ...
        for (const road of this.connectedRoads) {
            const other_interseption_for_this_road = road.startIntersection === this ? road.endIntersection : road.startIntersection;
            const dx = other_interseption_for_this_road.x - this.x;
            const dy = other_interseption_for_this_road.y - this.y;
